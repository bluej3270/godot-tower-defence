...
#shooting variables
var instance
var shot = load("res://Objects/Towers/Shots/MiniMissle.tscn")
...
func _process(_delta):
	if building:
		...
		if can_build:
			...
			$Rotate/Launcher.modulate = Color(0.0, 1.0, 0.0, 0.6)
		else:
			$Base.modulate = Color(1.0, 0.0, 0.0, 0.6)
			$Rotate/Launcher.modulate = Color(1.0, 0.0, 0.0, 0.6)
		if Input.is_action_just_pressed("click") and can_build:
			...
			$Rotate/Launcher.modulate = Color(1.0, 1.0, 1.0, 1.0)
		...
		if !current_target: #if we don't currently have a target
			...
			if (!current_target): #if the target has already been destroied
				...
				if is_instance_valid(current_target):
					...
					$Rotate.rotation = (target_position - position).angle() + deg2rad(90)
...
func _on_ShootTimer_timeout():
	...
	if current_target:
		...
		instance.position = $Rotate/Launcher/ShotPosition1.get_global_transform().origin
		...
		instance = shot.instance()
		instance.set_target(current_target)
		instance.position = $Rotate/Launcher/ShotPosition2.get_global_transform().origin
		get_parent().add_child(instance)
