...
#shooting variables
var instance
var shot = load("res://Objects/Towers/Shots/Missle.tscn")
...
func _process(_delta):
	if building:
		...
		if can_build:
			...
			$Rotate.modulate = Color(0.0, 1.0, 0.0, 0.6)
		else:
			$Base.modulate = Color(1.0, 0.0, 0.0, 0.6)
			$Rotate.modulate = Color(1.0, 0.0, 0.0, 0.6)
		if Input.is_action_just_pressed("click") and can_build:
			...
			$Rotate.modulate = Color(1.0, 1.0, 1.0, 1.0)
			$Agro/Sprite.hide()
	else:
...
func _on_ShootTimer_timeout():
		...
		if loaded:
			if current_target:
				...
				instance.position = $Rotate/Launcher/ShotPosition.get_global_transform().origin
				get_parent().add_child(instance)
				loaded = false
				$Rotate/UnloadedLauncher.show()
				$Rotate/Launcher.hide()
		else:
			loaded = true
			$Rotate/UnloadedLauncher.hide()
			$Rotate/Launcher.show()
